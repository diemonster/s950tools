...
// (slot, name, totalWords) and returns the cached words12 buffer if
// any. A clean miss returns (nil, nil) — the frontend treats that
// the same as the cache being uninitialised. Errors are reserved for
// genuine I/O problems.
func (a *App) GetCachedWaveform(slot int, name string, totalWords int) ([]uint16, error) {
	c := a.wavecache()
	if c == nil {
		return nil, nil
	}
	words, err := c.Get(waveformcache.Key{Slot: slot, Name: name, TotalWords: totalWords})
	if err != nil {
		if errors.Is(err, os.ErrNotExist) || errors.Is(err, waveformcache.ErrCorrupt) {
			// Both are "no usable cache" — silent miss is the
			// right answer for the GUI.
			return nil, nil
		}
		return nil, err
	}
...
// totalWords) tuple. Called by the Apply Slicing success path right
// after the in-memory attach. Returns nil on success; non-fatal
// errors are returned so the frontend can decide whether to surface
// them (currently it just swallows them — a missing cache write
// degrades to the synthetic waveform on next launch, not a crash).
func (a *App) PutCachedWaveform(slot int, name string, totalWords int, words []uint16) error {
	c := a.wavecache()
	if c == nil {
		return nil
	}
	return c.Put(waveformcache.Key{Slot: slot, Name: name, TotalWords: totalWords}, words)
}
