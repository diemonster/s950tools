...
export namespace main {
	...
	export class ExportImageSample {
	    params: protocol.SampleParams;
	    words: number[];
	
	    static createFrom(source: any = {}) {
	        return new ExportImageSample(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.params = this.convertValues(source["params"], protocol.SampleParams);
	        this.words = source["words"];
	    }
	...
	export class ImportedSample {
	    ...
	    static createFrom(source: any = {}) {
	        return new ImportedSample(source);
	    }
	...
	export class ImportImageResult {
	    path: string;
	    ...
	    static createFrom(source: any = {}) {
	        return new ImportImageResult(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.path = source["path"];
	        this.samples = this.convertValues(source["samples"], ImportedSample);
	        this.programs = this.convertValues(source["programs"], protocol.ProgramJSON);
	...
	export class MidiThruCapabilities {
	    virtualSupported: boolean;
	    virtualPortName: string;
	    hint: string;
	
	    static createFrom(source: any = {}) {
	        return new MidiThruCapabilities(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.virtualSupported = source["virtualSupported"];
	        this.virtualPortName = source["virtualPortName"];
